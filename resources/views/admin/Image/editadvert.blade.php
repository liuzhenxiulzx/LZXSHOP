...
<div class="clearfix" id="add_picture">
	...
	<form action="{{route('doeditadvert',['id'=>$image->id])}}" method="post" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
    {{csrf_field()}}
		<div class="clearfix cl">
         	<label class="form-label col-2">所属分类:</label>
			 <div class="formControls col-2">
			 <select name="catName" id="select">
                    @foreach($imgcate as $k)
                        @if($image->imgclass == $k->catName )
			 		  <option value="{{$k->id}}" selected = "selected">{{$k->catName}}</option>
                       @else
                       <option value="{{$k->id}}">{{$k->catName}}</option>
                        @endif
                    @endforeach
			 </select>
			</div> 
		...
		<div class="clearfix cl">
			 ...
			 <div class="formControls col-2">
			 	<input type="text" name="size_w" value = "{{$image->size_w}}" >  * <input type="text" name="size_h" value="{{$image->size_h}}">					 
			</div>
		</div>
        <!-- <div class="clearfix cl">
