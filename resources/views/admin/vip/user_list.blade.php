...
<div class="page-content clearfix">
    ...
    <div class="search_style">
     ...
     <div class="border clearfix">
       <span class="l_f">
        <a href="{{route('Adminaddvip')}}"  class="btn btn-warning"><i class="icon-plus"></i>添加用户</a>
    ...
    @foreach($vip as $v)
		<tr>
          ...
          <td>{{$v->phone}}</td>
          <td>{{$v->userEmail}}</td>
          <td class="text-l">{{$v->home_address}}</td>
          <td>{{$v->created_at}}</td>
       
          <td>{{$v->grade_ID}}</td>
        
          <td class="td-manage">
            <a title="编辑"  href="{{route('Admineditvip',['id'=>$v->id])}}"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
            <a title="删除"  href="{{route('Admindelectvip',['id'=>$v->id])}}"  onclick="return confirm('确定要删除吗？');" class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120"></i></a>
          </td>
		</tr>
     @endforeach
      </tbody>
