...
<div class="margin clearfix">
 <div class="sort_style">
  <div class="border clearfix">
       <span class="l_f">
        <a href="{{route('Adminaddblogcate')}}"id="add_page" class="btn btn-warning" onclick="add_article_sort()"><i class="fa fa-plus"></i> 添加分类</a>
       ...
       <thead>
        ...
        @foreach($blogcate as $v)
         <tr>
          ...
          <!-- <td>1</td> -->
          <td>{{$v->catName}}</td>
          <td class="displayPart" displayLength="60">{{$v->abstract}}</td>
          <td>{{$v->created_at}}</td>
          <td class="td-manage">   
           <a title="编辑" href="{{route('Admineditblogcate',['id'=>$v->id])}}"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>      
           <a title="删除" href="{{route('Admindeletblogcate',['id'=>$v->id])}}"  onclick="return confirm('确定要删除吗！')" class="btn btn-xs btn-danger" ><i class="fa fa-trash  bigger-120"></i></a>
          </td>
         </tr>
        @endforeach
        </tbody>
